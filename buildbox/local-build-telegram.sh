...
if [ "$1" == "hockeyapp" ] || [ "$1" == "testinghockeyapp" ]; then
	CERTS_PATH=$BUILD_PATH/codesigning_data/certs
	PROFILES_PATH=$BUILD_PATH/codesigning_data/profiles
...
SOURCE_PATH=$BUILD_PATH/telegram-ios
...
cp -r $BUILD_PATH/codesigning_teams $SOURCE_PATH/buildbox/transient-data/teams
...
mkdir -p $BUILD_PATH/Library/MobileDevice/Provisioning Profiles

for f in $(ls "$PROFILES_PATH"); do
	PROFILE_PATH="$PROFILES_PATH/$f"
	uuid=`grep UUID -A1 -a "$PROFILE_PATH" | grep -io "[-A-F0-9]\{36\}"`
	cp -f "$PROFILE_PATH" $BUILD_PATH/Library/MobileDevice/Provisioning Profiles/$uuid.mobileprovision
done
...
if [ -d "$BUCK_DIR_CACHE" ]; then
	sudo chown mac-zen "$BUCK_DIR_CACHE"
fi
...
OUTPUT_PATH=$BASE_DIR/build/artifacts
rm -rf "$OUTPUT_PATH"
mkdir -p "$OUTPUT_PATH"
