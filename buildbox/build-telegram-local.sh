...
BUILD_PATH=$HOME/telegram-ios-build/$VM_NAME
mkdir -p $BUILD_PATH

mkdir -p $BUILD_PATH/codesigning_data
mkdir -p $BUILD_PATH/codesigning_teams

cp -r $BUILDBOX_DIR/$CODESIGNING_SUBPATH $BUILD_PATH/codesigning_data
cp -r $BUILDBOX_DIR/$CODESIGNING_TEAMS_SUBPATH $BUILD_PATH/codesigning_teams
...
if [ "$BUILD_CONFIGURATION" == "verify" ]; then
    mkdir -p $BUILD_PATH/telegram-ios-shared/fastlane; echo '' > $BUILD_PATH/telegram-ios-shared/fastlane/Fastfile
...
cp $BUILDBOX_DIR/guest-build-telegram.sh $BUILD_PATH/
cp $BUILDBOX_DIR/transient-data/source.tar $BUILD_PATH/
...
export TELEGRAM_BUILD_APPSTORE_PASSWORD=\"$TELEGRAM_BUILD_APPSTORE_PASSWORD\";
export TELEGRAM_BUILD_APPSTORE_TEAM_NAME=\"$TELEGRAM_BUILD_APPSTORE_TEAM_NAME\";
export TELEGRAM_BUILD_APPSTORE_USERNAME=\"$TELEGRAM_BUILD_APPSTORE_USERNAME\";
export BUILD_NUMBER=\"$BUILD_NUMBER\";
export COMMIT_ID=\"$COMMIT_ID\";
export COMMIT_AUTHOR=\"$COMMIT_AUTHOR\";
export BUCK_HTTP_CACHE=\"$BUCK_HTTP_CACHE\";
mkdir -p ~/cache/telegram-ios;
export BUCK_DIR_CACHE="~/cache/telegram-ios";
export BUILD_PATH=$BUILD_PATH;
export BUCK_CACHE_MODE=\"$BUCK_CACHE_MODE\"; $GUEST_SHELL -l $BUILDBOX_DIR/local-build-telegram.sh $BUILD_CONFIGURATION;

OUTPUT_PATH=$BUILD_PATH/build/artifacts
# rm -rf "$OUTPUT_PATH"
# mkdir -p "$OUTPUT_PATH"
