                    ...
                    let beginShare: () -> Void = {
                        ...
                        if let appEnv = environment as? ShareControllerAppEnvironment,
                           let unlockedHiddenAccountRecordId = appEnv.sharedContext.accountManager.hiddenAccountManager.unlockedAccountRecordId {
                            filteredAccounts = otherAccounts.filter { $0.account.accountId == unlockedHiddenAccountRecordId }
                        } else {
                            filteredAccounts = otherAccounts.filter { item in
                                if let appContext = item.account as? ShareControllerAppAccountContext {
                                    return !appContext.context.account.isHidden
                                } else {
                                    return true
                                }
                            }
