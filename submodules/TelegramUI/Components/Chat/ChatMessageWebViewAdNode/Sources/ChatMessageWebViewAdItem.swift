...
@available(iOS 15.0, *)
public final class ChatMessageWebViewAdItem: ListViewItem {
    public let chatLocation: ChatLocation
    public let context: AccountContext
    public let controllerInteraction: ChatControllerInteraction
    public let presentationData: ChatPresentationData
    
    public init(chatLocation: ChatLocation, context: AccountContext, controllerInteraction: ChatControllerInteraction, presentationData: ChatPresentationData) {
    ...
    public func nodeConfiguredForParams(
        ...
        let configure = { () -> Void in
            let node = ChatMessageWebViewAdNode(rotated: self.controllerInteraction.chatIsRotated)
            node.setupItem(self)
    ...
    public func updateNode(async: @escaping (@escaping () -> Void) -> Void, node: @escaping () -> ListViewItemNode, params: ListViewItemLayoutParams, previousItem: ListViewItem?, nextItem: ListViewItem?, animation: ListViewItemUpdateAnimation, completion: @escaping (ListViewItemNodeLayout, @escaping (ListViewItemApply) -> Void) -> Void) {
        Queue.mainQueue().async {
            if let nodeValue = node() as? ChatMessageWebViewAdNode {
