...
private func preparedTransition(
    ...
    let insertions = indicesAndItems.map {
        ListViewInsertItem(
            index: $0.0,
            previousIndex: $0.2,
            item: $0.1.item(context: context, presentationData: presentationData, openMessage: openMessage),
            directionHint: nil
        )
    }
...
public final class SpyOnFriendsHeaderItem: ListViewItem, ItemListItem {
    ...
    public func nodeConfiguredForParams(async: @escaping (@escaping () -> Void) -> Void, params: ListViewItemLayoutParams, synchronousLoads: Bool, previousItem: ListViewItem?, nextItem: ListViewItem?, completion: @escaping (ListViewItemNode, @escaping () -> (Signal<Void, NoError>?, (ListViewItemApply) -> Void)) -> Void) {
        let configure = { () -> Void in
            let node = SpyOnFriendsHeaderNode()
            node.setupItem(self)
...
class SpyOnFriendsHeaderNode: ListViewItemNode {
    ...
    required init() {
        let headerView = SpyOnFriendsHeaderView()
...
//            headerView.layoutIfNeeded()
            ...
            let headerInsets = UIDevice.current.orientation == .portrait ?
                UIEdgeInsets.top(12).horizontal(16) :
                UIEdgeInsets.top(12).horizontal(59)
            ...
            let size = CGSize(
                ...
                height: headerSize.height + headerInsets.top + headerInsets.bottom
            )
...
//            messagesView.layoutIfNeeded()
            ...
            let messagesInsets = UIDevice.current.orientation == .portrait ?
                UIEdgeInsets.top(32).horizontal(16) :
                UIEdgeInsets.top(32).horizontal(59)
            let messagesSize = messagesView.systemLayoutSizeFitting(
                UIView.layoutFittingExpandedSize
            )
            let size = CGSize(
                width: params.width - (messagesInsets.left + messagesInsets.right),
                height: messagesSize.height + messagesInsets.top + messagesInsets.bottom
            )
...
class SpyOnFriendsUnlockNode: ListViewItemNode {
    ...
    func asyncLayout() -> (_ item: SpyOnFriendsUnlockItem, _ params: ListViewItemLayoutParams, _ mergedTop: Bool, _ mergedBottom: Bool, _ dateAtBottom: Bool) -> (ListViewItemNodeLayout, (ListViewItemUpdateAnimation) -> Void) {
        return { [weak self] item, params, mergedTop, mergedBottom, dateHeaderAtBottom -> (ListViewItemNodeLayout, (ListViewItemUpdateAnimation) -> Void) in
            ...
            unlockView.rotate()
            self.unlockView.updateConstraintsIfNeeded()
            
            let unlockInsets = UIDevice.current.orientation == .portrait ?
                UIEdgeInsets.vertical(32).horizontal(16) :
                UIEdgeInsets.vertical(32).horizontal(59)
            ...
            let size = CGSize(
                ...
                height: unlockSize.height + unlockInsets.top + unlockInsets.bottom
            )
