...
final class ChatListTitleView: UIView, NavigationBarTitleView, NavigationBarTitleTransitionNode {
        ...
        didSet {
            if self.title != oldValue {
                ...
                if !self.nicegramSettings.showGmodIcon {
                    ...
                    self.gmodNode.isHidden = false
                    self.gmodButton.isHidden = false
                }
    ...
    func updateLayout(size: CGSize, clearBounds: CGRect, transition: ContainedViewLayoutTransition) {
        ...
        let gmodFrame = CGRect(origin: CGPoint(x: clearBounds.maxX - 9.0 - self.gmodNode.bounds.width - (self.proxyButton.isHidden ? 0.0 : 9.0) - (self.proxyButton.isHidden ? 0.0 : self.proxyNode.bounds.width), y: floor((size.height - gmodNode.bounds.height) / 2.0)), size: gmodNode.bounds.size)
